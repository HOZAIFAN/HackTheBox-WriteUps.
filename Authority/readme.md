...
* **PWM Configuration Mode → Fake LDAP Server → Cleartext Credentials** — Discover the PWM instance on port 8443 running in open configuration mode. Point its LDAP URL at an attacker-controlled listener and capture `svc_ldap`'s cleartext password when PWM attempts to bind.
...
do_connect: Connection to 10.129.229.56 failed (Error NT_STATUS_RESOURCE_NAME_NOT_FOUND)
...
The `!vault` values are AES256-encrypted with a passphrase, not directly usable. I convert the vault blob into a crackable hash format and run it offline through Hashcat:

```shell
hyena@hyena$ ansible2john main.yml > vault.hash
hyena@hyena$ hashcat -m 16900 vault.hash /usr/share/wordlists/rockyou.txt --force

$ansible$0*...:VaultPass_2023!
...
With the vault passphrase `VaultPass_2023!`, I decrypt the file and recover the plaintext PWM and LDAP credentials:

```shell
hyena@hyena$ ansible-vault decrypt main.yml --vault-password-file <(echo 'VaultPass_2023!')
Decryption successful

hyena@hyena$ cat main.yml
...
This confirms the LDAP bind DN for `svc_ldap` and highlights a certificate validation issue I need to resolve before I can interact with LDAPS directly.

I first try the cracked `pwm_admin_password` directly against the PWM self-service login form, expecting it to double as an end-user credential:

<img src="POC/PasswordMenagerButPasswordDidNotWork.png" alt="error loading image">

The login fails — this password belongs to the PWM **configuration/admin** context, not a directory account usable at the self-service login. Browsing the site unauthenticated as a would-be Administrator, PWM discloses additional internal configuration details rather than a generic error:

<img src="POC/ForAdminWeAreShownThis.png" alt="error loading image">

This nudges me toward the configuration manager itself rather than the end-user login flow, so I resolve the LDAPS trust issue first before going further.

---

## Certificate Extraction for LDAPS

I extract the LDAPS certificate so my tooling can trust the connection:
...
## PWM Configuration Manager — Admin Access

I navigate to the configuration manager:

```
https://10.129.229.56:8443/pwm/private/configmanager
```

After multiple attempts, the correct configuration-editor password is recovered from the decrypted Ansible vault values.

<img src="POC/LogIntoConfigurationPage.png" alt="error loading image">

With `svc_pwm:pWm_@dm!N_!23`, I log in successfully:
...
```
Anonymous SMB Enumeration → Development Share → Ansible Vault
   → PWM Configuration Mode → Fake LDAP Server → svc_ldap Cleartext Credentials
   → WinRM Foothold → user.txt
   → AD CS Enumeration → ESC1 (CorpVPN template)
